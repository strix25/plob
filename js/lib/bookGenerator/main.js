...
function Book(id, name, author, gender, genre, publish_date) {
	this.id = id;
	this.name = name;
	this.author = author;
	this.gender = gender;
...
//$(document).ready(function () {
	...
	var generateNumberPerCycle = 1000000;
	...
	//for (var outsideCycleCounter = 1; outsideCycleCounter <= 10; outsideCycleCounter++) {
		...
		for (var countCycle = 0; countCycle < generateNumberPerCycle; countCycle++) {
			for (var genderCount = 0; genderCount < genderType.length - 1; genderCount++) {
				...
				//Edited to use includes
				// if (existingbookName.includes(bookName)) {
				...
				// 	//Edited to use includes
				// 	while(existingbookName.includes(bookName)) {
				// 		i++;
				// 		bookName = bookName + " " + i;
				// 	}
				...
				var bookObj = new Book(countCycle, bookName, bookAuthor.name, bookAuthor.gender, bookGenre, bookPublishDate);
	...
	fs.writeFile("../../../boooks.js", JSON.stringify(AllBooks), function(err) {
		if(err) {
